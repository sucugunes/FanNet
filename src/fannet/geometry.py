...
def compute_operators(verts, faces, k_eig, normals=None, spoke_length=[0.00,0.01,0.02,0.03]):
    """
    ...
    mesh = om.TriMesh(verts_np, faces_np)
    num_vertices = verts_np.shape[0]
    MAX_NUM_SPOKES = 6
    fanIndices = np.zeros(shape=(num_vertices, 6 * len(spoke_length) + 1), dtype=int)
    
    
    for idx_num_vertices in tqdm(range(num_vertices)):

        for idx, spoke_length_idx in enumerate(spoke_length):
            ##### orjinalde bu satir acik
            paths = find_paths_adaptivefan(mesh, spoke_length_idx, idx_num_vertices)
            #####
            # pretend all nodes have 6 neighbors
            #vertex_neighbors = mesh.vertex_neighbors[idx_num_vertices]
            vertex_neighbors = mesh.vv_indices()[idx_num_vertices]
            vertex_neighbors = vertex_neighbors[~np.isin(vertex_neighbors, -1)]
            '''
            if len(vertex_neighbors) == 2:
                lst = [paths[0][0]] + paths[0][1:] + paths[1][1:] + paths[0][1:] + paths[1][1:] + paths[0][1:] + \
                      paths[1][1:]
            elif len(vertex_neighbors) == 3:
                lst = [paths[0][0]] + paths[0][1:] + paths[1][1:] + paths[2][1:] + paths[0][1:] + paths[1][1:] + \
                      paths[2][1:]
            elif len(vertex_neighbors) == 4:
                lst = [paths[0][0]] + paths[0][1:] + paths[1][1:] + paths[2][1:] + paths[3][1:] + paths[0][1:] + \
                      paths[1][1:]
            elif len(vertex_neighbors) == 5:
                lst = [paths[0][0]] + paths[0][1:] + paths[1][1:] + paths[2][1:] + paths[3][1:] + paths[4][1:] + \
                      paths[0][1:]
            else:
                lst = [paths[0][0]] + paths[0][1:] + paths[1][1:] + paths[2][1:] + paths[3][1:] + paths[4][1:] + \
                      paths[5][1:]
                      
            lst = [paths[0][0]] #ustteki eliflerle aynı seyi yapiyor
            for i in range(6):
                index = int(i % len(vertex_neighbors))
                lst = lst + paths[index][1:]
            '''
            lst = []


            for idx_max_num_spokes in range(MAX_NUM_SPOKES):
                #import pdb; pdb.set_trace()

                if len(vertex_neighbors) == 0:
                    fanIndices[idx_num_vertices, :] = idx_num_vertices    
                else:

                    index = int(idx_max_num_spokes % len(vertex_neighbors))

                #### orjinalde bu satir acik
                    lst.append(paths[index][1:])
                    fanIndices[idx_num_vertices, 0] = paths[0][0]
                #####
                #if (idx == 0):
                #### orjinalde bu satir acik
                    fanIndices[idx_num_vertices, (idx + 1) + (idx_max_num_spokes * len(spoke_length))] = lst[idx_max_num_spokes][-1] #orjinalde bu satir acik asagisi kapali
                #####
                #fanIndices[idx_num_vertices, (idx + 1) + (idx_max_num_spokes * len(spoke_length))] = idx_num_vertices  #komsusuz yapmak icin bu satir
            #import pdb
            #pdb.set_trace()
                #else:
                #    fanIndices[idx_num_vertices, idx * idx_max_num_spokes + 1] = lst[idx_max_num_spokes][-1]

            #indices_temp.append(lst).
            #print(lst)
        #print(fanIndices[idx_num_vertices])
    
    fanIndices_np = fanIndices.astype(np.int32)

    #print(fanIndices_np.shape)
    #print(fanIndices_np)
    ...
    fanIndices = torch.from_numpy(fanIndices_np).to(device=device, dtype=int)

    return frames, massvec, L, evals, evecs, gradX, gradY, fanIndices


def get_all_operators(verts_list, faces_list, k_eig, op_cache_dir=None, normals=None, spoke_length=[0.00,0.01,0.02,0.03]):
    N = len(verts_list)
    ...
    for num, i in enumerate(inds):
        ...
        if normals is None:
            outputs = get_operators(verts_list[i], faces_list[i], k_eig, op_cache_dir, spoke_length=spoke_length)
        else:
            outputs = get_operators(verts_list[i], faces_list[i], k_eig, op_cache_dir, normals=normals[i], spoke_length=spoke_length)
        frames[i] = outputs[0]
        massvec[i] = outputs[1]
        L[i] = outputs[2]
        evals[i] = outputs[3]
        ...
        gradX[i] = outputs[5]
        gradY[i] = outputs[6]
        fanIndices[i] = outputs[7]
        
    return frames, massvec, L, evals, evecs, gradX, gradY, fanIndices

def get_operators(verts, faces, k_eig=128, op_cache_dir=None, normals=None, overwrite_cache=False, spoke_length=[0.00,0.01,0.02,0.03]):
    """
    ...
    if op_cache_dir is not None:
        ...
        while True:
            ...
            try:
                ...
                fanIndices_np = npzfile["fanIndices"]
    ...
    if not found:
        ...
        frames, mass, L, evals, evecs, gradX, gradY, fanIndices = compute_operators(verts, faces, k_eig, normals=normals, spoke_length=spoke_length)
        ...
        if op_cache_dir is not None:
            ...
            fanIndices_np = toNP(fanIndices).astype(np.int32)
            ...
            np.savez(search_path,
                     ...
                     fanIndices=fanIndices_np.astype(np.int32),
                     )

    return frames, mass, L, evals, evecs, gradX, gradY, fanIndices
...
def calculate_unit_vector(v): #v is a 3-D vector
    v_norm = math.sqrt(v[0]*v[0] + v[1]*v[1] + v[2]*v[2])
    v_unit = v/v_norm
    return v_unit


# choose next vertex - om version
def select_next_vertex(v1, v2, v2_neighbors, mesh):
    dps = []
    for n in v2_neighbors:
        dp = np.dot(calculate_unit_vector(mesh.points()[v1] - mesh.points()[v2]),
                    calculate_unit_vector(mesh.points()[n] - mesh.points()[v2]))
        dps.append(dp)

    # print(dps)
    return dps.index(min(dps))
'''
# choose next vertex - tm version
def select_next_vertex(v1, v2, v2_neighbors, mesh):
    dps = []
    for n in v2_neighbors:
        dp = np.dot(calculate_unit_vector(mesh.vertices[v1] - mesh.vertices[v2]),
                    calculate_unit_vector(mesh.vertices[n] - mesh.vertices[v2]))
        dps.append(dp)
...
# find paths - om version current one in use
def find_paths(mesh, spoke_length, s_v): # s_v : starting vertex
    paths = []
    vertex_neighbors = mesh.vv_indices()[s_v]
    vertex_neighbors = vertex_neighbors[~np.isin(vertex_neighbors, -1)]
    n_o_p = len(vertex_neighbors) # number of paths
    for i in range(n_o_p):
        current_neighbor = vertex_neighbors[i]
        path = [s_v, current_neighbor]
        for j in range(spoke_length-1):
            neighbors = mesh.vv_indices()[current_neighbor]
            neighbors = neighbors[~np.isin(neighbors, -1)]
            idx = select_next_vertex(path[j], path[j+1], neighbors, mesh)
            path.append(neighbors[idx])
            current_neighbor = neighbors[idx]
        paths.append(path)
    return paths



def find_paths_adaptivefan(mesh, spoke_length, s_v): # s_v : starting vertex
    MAX_ITER = 20
    paths = []
    vertex_neighbors = mesh.vv_indices()[s_v]
    vertex_neighbors = vertex_neighbors[~np.isin(vertex_neighbors, -1)]
    ...
    for i in range(n_o_p):
        ...
        path = [s_v, current_neighbor]
        n_iter = 0
        path_length = np.linalg.norm(mesh.points()[path[0]]-mesh.points()[path[1]])
        while path_length < spoke_length and n_iter < MAX_ITER:
            neighbors = mesh.vv_indices()[current_neighbor]
            neighbors = neighbors[~np.isin(neighbors, -1)]
            idx = select_next_vertex(path[n_iter], path[n_iter+1], neighbors, mesh)
            path.append(neighbors[idx])
            path_length += np.linalg.norm(mesh.points()[current_neighbor]-mesh.points()[neighbors[idx]])
            n_iter += 1
            current_neighbor = neighbors[idx]
        paths.append(path)
    return paths
...
def find_paths_adaptivefan(mesh, spoke_length, s_v): # s_v : starting vertex
    MAX_ITER = 20
    paths = []
    vertex_neighbors = mesh.vertex_neighbors[s_v]
    #vertex_neighbors = vertex_neighbors[~np.isin(vertex_neighbors, -1)]
    n_o_p = len(vertex_neighbors) # number of paths
    for i in range(n_o_p):
        current_neighbor = vertex_neighbors[i]
        path = [s_v, current_neighbor]
        n_iter = 0
        path_length = np.linalg.norm(mesh.vertices[path[0]]-mesh.vertices[path[1]])
        while path_length < spoke_length and n_iter < MAX_ITER:
            neighbors = mesh.vertex_neighbors[current_neighbor]
            #neighbors = neighbors[~np.isin(neighbors, -1)]
            idx = select_next_vertex(path[n_iter], path[n_iter+1], neighbors, mesh)
            path.append(neighbors[idx])
            path_length += np.linalg.norm(mesh.vertices[current_neighbor]-mesh.vertices[neighbors[idx]])
            n_iter += 1
            current_neighbor = neighbors[idx]
        paths.append(path)
    return paths  
